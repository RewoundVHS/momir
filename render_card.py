...
if found_card == 'Error':
    ...
    # Print the card data
    print("Name:", found_card['name'])
    print("Mana Cost:", found_card['mana_cost'])
    print("Type Line:", found_card['type_line'])
    print("Oracle Text:", found_card['oracle_text'])
    print("Power/Toughness:", found_card['power_toughness'])
    print("Art URL:", found_card['art'])
 
    print("Downloading art...")
    response = get(found_card['art'])
...
def main():
    # Fit text sizes 
    name_size = fit_text_width(found_card["name"], FONT_NAME, 48, 28, NAME_MAX_WIDTH)
    type_size = fit_text_width(found_card["type_line"], FONT_TYPE, 36, 20, TEXT_WIDTH)
    oracle_size = fit_text_height(found_card["oracle_text"], FONT_RULES, 30, 12, TEXT_WIDTH, ORACLE_HEIGHT)
    ...
    subprocess.run([
        ...
        "-gravity", "NorthWest",
        "-annotate", "+60+10", found_card["name"],
        "card.png"
    ], check=True)
    ...
    subprocess.run([
        ...
        "-pointsize", "36",
        "-gravity", "NorthEast",
        "-annotate", "+60+10", found_card["mana_cost"],
        "card.png"
    ], check=True)
    ...
    subprocess.run([
        ...
        "-gravity", "NorthWest",
        "-annotate", "+60+560", found_card["type_line"],
        "card.png"
    ], check=True)
    ...
    subprocess.run([
        ...
        "-background", "white",
        f"caption:{found_card['oracle_text']}",
        "-gravity", "NorthWest",
    ...
    subprocess.run([
        ...
        "-pointsize", "36",
        "-gravity", "SouthEast",
        "-annotate", "+60+80", found_card["power_toughness"],
        "output.png"
    ], check=True)
